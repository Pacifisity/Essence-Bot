...
class Party(commands.Cog, app_commands.Group):
    ...
    async def info(self, interaction: discord.Interaction, party: str = None):
        ...
        with sqlite3.connect('DB Storage/essence.db') as db:
            ...
            cursor.execute(f"SELECT essence_points FROM server WHERE party = ?", (party,))
    ...
    async def join(self, interaction: discord.Interaction, party: str = None):
        ...
        with sqlite3.connect('DB Storage/essence.db') as db:
            cursor = db.cursor()
            cursor.execute(f"SELECT party FROM users WHERE member_id = ?", (member.id,))
    ...
    async def daily(self, interaction: discord.Interaction):
        ...
        with sqlite3.connect('DB Storage/essence.db') as db:
            ...
            if brain_bean == None:
                brain_bean = 0
            cursor.execute(f"SELECT daily FROM users WHERE member_id = ?", (member.id,))
