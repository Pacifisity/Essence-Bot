...
class Item(commands.Cog, app_commands.Group):
    ...
    async def use(self, interaction: discord.Interaction, item: str):
        ...
        embed = discord.Embed(
            description=f"This command is a work in progress")
    ...
    async def shop(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="**Item Shop**", description="You can purchase items in the shop with party points (PP)", color=0x800080)
        embed.set_author(name=(f"{member.nick}"),
                         icon_url=member.display_avatar)
        embed.add_field(name="Brain Bean - 10 PP",
                        value="A magical fruit, the more you eat the more ap you get", inline=False)
        embed.add_field(name="Impurity Orb - 5 PP",
                        value="Smear a users name in the ultimate color of shame", inline=False)
        ...
        select = discord.ui.Select(
            options=[
                discord.SelectOption(
                    label="Brain Bean - 10 PP", description="A magical fruit, the more you eat the more ap you get."),
                discord.SelectOption(
                    label="Impurity Orb - 5 PP", description="Smear a users name in the ultimate color of shame."),
            ]
        )
        ...
        async def user_response(interaction: discord.Interaction):
            ...
            with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
                ...
                if brain_bean == None:
                    brain_bean = 0
                cursor.execute(
                    f"SELECT impurity_orb FROM users WHERE member_id = ?", (member.id,))
                impurity_orb = cursor.fetchone()
                impurity_orb = impurity_orb[0]
            ...
            if select.values[0] == "Brain Bean - 10 PP":
                if party_points >= 10:
                    ...
                    db.commit()
                    embed.add_field(
                        name=f"-", value=f"<@{member.id}> bought `{select.values[0]}`")
                else:
                    embed.add_field(
                        name="-", value=f"{member.nick} doesn't have enough PP to buy {select.values[0]}")
        ...
        await command_logs.send(embed=embed, view=view)
    ...
    async def inventory(self, interaction: discord.Interaction):
        ...
        with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
            cursor = db.cursor()
            cursor.execute(
                f"SELECT brain_bean, crystal_of_power, joker_card, easter_egg, impurity_orb FROM users WHERE member_id = ?", (member.id,))
            inventory = cursor.fetchone()
            items = ["Brain Bean", "Crystal of Power",
                     "Joker Card", "Easter Egg", "Impurity Orb"]
            for i in enumerate(inventory):
                if not i[1] == None:
                    embed.add_field(
                        name=f"{items[i[0]]}:", value=f"{i[1]}", inline=False)
            embed.set_author(
                name=(f"{member.nick}'s Command:"), icon_url=member.display_avatar)
            await interaction.response.send_message(embed=embed, ephemeral=True)
            await command_logs.send(embed=embed)
