...
class Essence(commands.Cog):
    ...
    async def hide(self, interaction: discord.Interaction, options: str = None):
        ...
        with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
            ...
            if options == None:
                embed = discord.Embed(
                    description=f"about = Your about me section\ninfo = Your account information section", colour=0x800080)
            elif options == 'about':
                if hide_about == None or hide_about == 0:
                    sql = "UPDATE users SET hide_about = ? WHERE member_id = ?"
                    val = (1, member.id)
                    embed = discord.Embed(
                        description=f"You have hidden your about section.", colour=0x800080)
                else:
                    sql = "UPDATE users SET hide_about = ? WHERE member_id = ?"
                    val = (0, member.id)
                    embed = discord.Embed(
                        description=f"Your about section is visible now.", colour=0x800080)
                ...
                if hide_info == None or hide_info == 0:
                    ...
                    val = (1, member.id)
                    embed = discord.Embed(
                        description=f"You have hidden your info.", colour=0x800080)
                else:
                    sql = "UPDATE users SET hide_info = ? WHERE member_id = ?"
                    val = (0, member.id)
                    embed = discord.Embed(
                        description=f"Your info is visible now.", colour=0x800080)
    ...
    async def notifications(self, interaction: discord.Interaction):
        ...
        with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
            cursor = db.cursor()
            cursor.execute(
                f'SELECT notifications FROM users WHERE member_id = ?', (member.id,))
            ...
            if notifications == None or notifications == 0:
                ...
                val = (1, member.id)
                embed = discord.Embed(
                    description=f"You have been signed up for notifications across Essence, if you ignore a notification it will not come up again.", colour=0x800080)
            else:
                sql = "UPDATE users SET notifications = ? WHERE member_id = ?"
                val = (0, member.id)
                embed = discord.Embed(
                    description=f"You have turned off notifications across Essence.", colour=0x800080)
            cursor.execute(sql, val)
            db.commit()
            embed.set_author(
    ...
    async def artist(self, interaction: discord.Interaction):
        member = interaction.user
        artist = discord.utils.get(
            interaction.guild.roles, id=758084699007746179)
        await member.add_roles(artist)
        embed = discord.Embed(
            description=f"You have been given the artist role, remember to follow the channel topics and look at pins to keep it clean.", colour=0x800080)
        embed.set_author(name=(f"{member.nick}'s Command"),
                         icon_url=member.display_avatar)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await self.command_logs.send(embed=embed)
    ...
    async def supporter(self, interaction: discord.Interaction):
        member = interaction.user
        supporter = discord.utils.get(
            interaction.guild.roles, id=741720677308039278)
        await member.add_roles(supporter)
        embed = discord.Embed(
            description=f"You have been given the supporter role, access to entertainment channels has been given", colour=0x800080)
        embed.set_author(name=(f"{member.nick}'s Command"),
                         icon_url=member.display_avatar)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await self.command_logs.send(embed=embed)
    ...
    async def nick(self, interaction: discord.Interaction, arg: str = None):
        ...
        embed = discord.Embed(
            description=f"Your nickname has been updated, {member.mention}", colour=0x800080)
        embed.set_author(name=(f"{member.nick}'s Command"),
                         icon_url=member.display_avatar)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await self.command_logs.send(embed=embed)
    ...
    async def suggest(self, interaction: discord.Interaction, suggestion: str = None):
        ...
        if suggestion == None:
            embed = discord.Embed(
                description=f"There seems to be a problem with your suggestion, {member}!", colour=0x800080)
        else:
            channel = self.bot.get_channel(927368289821294612)
            embed = discord.Embed(
                description=f"{suggestion} suggested by <@{member.id}>", colour=0x800080)
            await channel.send(embed=embed)
            embed = discord.Embed(
                description=f"Your suggestion has been sent to the organizer.", colour=0x800080)
        embed.set_author(name=(f"{member.nick}'s Command"),
                         icon_url=member.display_avatar)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await self.command_logs.send(embed=embed)
    ...
    async def ping(self, interaction: discord.Interaction):
        member = interaction.user
        embed = discord.Embed(
            description=f"Pong! {round(self.bot.latency * 1000)}ms", colour=0x800080)
        embed.set_author(name=(f"{member.nick}'s Command"),
                         icon_url=member.display_avatar)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await self.command_logs.send(embed=embed)
    ...
    async def setstatus(self, interaction: discord.Interaction, *, status: str):
        ...
        if len(status) <= 128:
            ...
            embed = discord.Embed(
                description="Bot status set to `Playing " + str(status) + "`", colour=0x800080)
            embed.set_author(
                name=(f"{member.nick}'s Command"), icon_url=member.display_avatar)
            with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
                cursor = db.cursor()
            ...
            embed = discord.Embed(
                description=f"Too many characters, the bot's status can't support more than 128 characters.", colour=0x800080)
            embed.set_author(
                name=(f"{member.nick}'s Command"), icon_url=member.display_avatar)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await self.command_logs.send(embed=embed)
    ...
    async def leaderboard(self, interaction: discord.Interaction, board: str = None):
        ...
        with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
            ...
            if board == None:
                embed = discord.Embed(
                    title="Leaderboard Ranking", colour=0x800080)
                for ranking in ["activity_points", "party_points", "user_count"]:
                    cursor.execute(
                        f"SELECT member_id FROM users ORDER BY {ranking} DESC")
                    ...
                    for i, (member_id,) in enumerate(leaderboard, start=1):
                        if member_id == member.id:
                            ...
                            if i <= 1:
                                ...
                                rank = f"#{i}"
                            embed.add_field(
                                name=ranking, value=rank, inline=False)
            elif board == 'pp':
                cursor.execute(
                    f"SELECT member_id, party_points, party FROM users GROUP BY member_id ORDER BY 2 DESC LIMIT 10")
                leaderboard = cursor.fetchall()
                cursor.execute(
                ...
                embed.add_field(name="Cumulative party points",
                                value=f"{leaderboard2[4][0]} {leaderboard2[4][1]} ₽₽ | {leaderboard2[2][0]} {leaderboard2[2][1]} ₽₽ | {leaderboard2[1][0]} {leaderboard2[1][1]} ₽₽ | {leaderboard2[3][0]} {leaderboard2[3][1]} ₽₽")
            elif board == 'ap':
                cursor.execute(
                    f"SELECT member_id, activity_points, party FROM users GROUP BY member_id ORDER BY 2 DESC LIMIT 10")
                leaderboard = cursor.fetchall()
                cursor.execute(
                    f"SELECT party, SUM(activity_points) FROM users GROUP BY party")
                leaderboard2 = cursor.fetchall()
                ...
                embed.add_field(name="Cumulative party points",
                                value=f"{leaderboard2[4][0]} {leaderboard2[4][1]} A₽ | {leaderboard2[2][0]} {leaderboard2[2][1]} A₽ | {leaderboard2[1][0]} {leaderboard2[1][1]} A₽ | {leaderboard2[3][0]} {leaderboard2[3][1]} A₽")
            elif board == 'ip':
                embed = discord.Embed(
                    description=f"Coming Soon", colour=0x800080)
            elif board == 'ep':
                cursor.execute(
                    f"SELECT party, essence_points FROM server ORDER BY 2 DESC LIMIT 4")
                leaderboard = cursor.fetchall()
                embed = discord.Embed(title="Essence Point Leaderboard",
                                      description=f":first_place: Party {str.upper(leaderboard[0][0])} with {leaderboard[0][1]} E₽!\n :second_place: Party {str.upper(leaderboard[1][0])} with {leaderboard[1][1]} E₽!\n :third_place: Party {str.upper(leaderboard[2][0])} with {leaderboard[2][1]} E₽!\n :medal: Party {str.upper(leaderboard[3][0])} with {leaderboard[3][1]} E₽!", colour=0x800080, timestamp=datetime.utcnow())
            elif board == 'c':
                cursor.execute(
                    f"SELECT member_id, user_count FROM users GROUP BY member_id ORDER BY 2 DESC LIMIT 10")
                leaderboard = cursor.fetchall()
