        ...
        with sqlite3.connect('DB Storage/essence.db') as db:
            ...
            with sqlite3.connect('DB Storage/essence.db') as db:
                ...
                for attribute in attributes:
                    cursor.execute(f'SELECT {attribute} FROM incremental WHERE member_id = ?', (member.id,))
...
def hp_suppresion(hp_lost, member):
    ...
    if character.character_level > max_world_level:
        hp_lost = (hp_lost / (1 + ((character.character_level - max_world_level) / 10)))
        if character.character_level > max_world_level + 10:
            hp_lost = hp_lost / 3 * (1 + ((character.character_level - max_world_level) / 10))
...
def xp_suppresion(xp_gained, member):
    ...
    max_world_level = 10 * character.character_world_level
    if character.character_level > max_world_level:
        xp_gained = xp_gained / (1 + ((character.character_level - max_world_level) / 10))
        if character.character_level > max_world_level + 10:
            xp_gained = xp_gained / 3 * (1 + ((character.character_level - max_world_level) / 10))
...
def character_update(hp_lost, xp_gained, energy_stones_gained, member):
    ...
    val = ((character.character_xp + xp_gained), (character.character_hp - hp_lost), (character.character_energy_stones + energy_stones_gained), member.id)
    ...
    val = ((character.stat_explorations + 1), (character.stat_xp_gained + xp_gained), (character.stat_damage_taken + hp_lost), (character.stat_energy_stones_gained + energy_stones_gained), member.id)
    database.cursor.execute(sql, val)
    database.db.commit()
...
def death_check(hp_lost, member):
    ...
    if character.character_hp <= 0:
        if round(character.character_xp - character.character_xp_required * 0.2) <= 0:
            ...
            experience_after_death = round(character.character_xp - character.character_xp_required * 0.2)
...
class Incremental(commands.Cog, app_commands.Group):
    ...
    async def send_message(self, interaction: discord.Interaction, embed):
        await interaction.response.send_message(embed=embed, ephemeral=True); await self.incremental_logs.send(embed=embed)
    ...
    async def register(self, interaction: discord.Interaction, *, character_name: str = None):
        ...
        if character.character_name == None:
            if not character.character_name == None:
                embed = default_embed(f"You've already started your journey, {character.character_name}", member=member)
                await self.send_message(interaction, embed)
            else:
            ...
            if character.character_name == None:
                ...
                val = (member.id, character_name, 1, 0, 1, 0, 100, 100, timestamp, 0, 0, 0, 0, 0)
                database.cursor.execute(sql, val)
                database.db.commit()
                embed = default_embed(f"Welcome to the game of Essence, your character {character_name} has been created.", member=member)
                await self.send_message(interaction, embed)
            else:
                embed = default_embed(f"You've already started your journey, {character_name[0]}", member=member)
                await self.send_message(interaction, embed)
    ...
    async def character(self, interaction: discord.Interaction):
        ...
        character_energy_stones = number_format(character.character_energy_stones)
        
        embed = default_embed(f"Character Information:", member=member)
        embed.add_field(name="Name", value=f"{character.character_name}", inline=True)
        embed.add_field(name="Level", value=f"{state_of_being} {character.character_level}", inline=True)
        embed.add_field(name="Experience", value=f"{character_xp} of {character_xp_required}", inline=True)
        embed.add_field(name="Health", value=f"{character.character_hp}%", inline=True)
    ...
    async def advance(self, interaction: discord.Interaction):
        ...
        database = Database()
        if character.character_name == None:
            embed = default_embed("You don't have a character yet, do '$register (character name)' to create a character!", member=member)
        ...
        if character.character_xp >= character.character_xp_required:
            embed = default_embed(f"{character.character_name} has advanced to {state_of_being} {character.character_level + 1}!", member=member)
            sql = "UPDATE incremental SET character_level = ?, character_xp = ?, character_xp_required = ? WHERE member_id = ?"
            val = (character.character_level + 1, 0, round(character.character_xp_required * 1.35), member.id)
            database.cursor.execute(sql, val)
            database.db.commit()
            await self.send_message(interaction, embed)
        else:
            embed = default_embed(f"{character.character_name} wasn't ready to advance and caused a backlash on their mind while trying.\n-10hp", member=member)
        ...
        if dead:
            embed = default_embed(f"{character.character_name} died while trying to advance, they feel weaker but they don't remember dying", member=member)
            await self.send_message(interaction, embed)
    ...
    async def recover(self, interaction: discord.Interaction):
        ...
        if character.cooldown_rest == None or character.cooldown_rest <= timestamp - 3600:
            ...
            embed = default_embed(f"Too much recovery at once could affect the healing process. You can recover again <t:{character.cooldown_rest + 3600}:R>", member=member)
        await self.send_message(interaction, embed)
    ...
    async def explore(self, interaction: discord.Interaction):
        ...
        embed = default_embed(f"**{character.character_name} went on an adventure to explore the world of Essence**", member=member)
        if character.character_world_level == 1:

            if exploration_number == 1:
                embed.add_field(name="-", value=f"{character.character_name} fell down a cliff", inline=False)
                cliff_scenario = random.randint(1,10)
                if cliff_scenario <= 7:
                    hp_lost = hp_suppresion(5, member)
                    embed.add_field(name="-", value=f"{character.character_name} hit their head on the way down, luckily, {character.character_name} didn't fall far\n-{hp_lost}%hp", inline=False)
                if cliff_scenario == 8 or cliff_scenario == 9:
                    energy_stones_gained = 3
                    embed.add_field(name="-", value=f"{character.character_name} caught a tree branch just in time and found some energy stones in the wall\n+{energy_stones_gained} Energy Stones", inline=False)
                if cliff_scenario == 10:
                    xp_gained = xp_suppresion(100, member)
                    energy_stones_gained = 20
                    embed.add_field(name="-", value=f"{character.character_name} found the cave of a forgotten warrior\n+{xp_gained}xp +{energy_stones_gained} Energy Stones", inline=False)
                    # first world key ----------------------------------------------------------------------------------------------------------------------------------------------------------------
                    if character.character.character_level > 9:
                        embed.add_field(name="-", value=f"{character.character_name} hears a deep voice coming from behind them, 'You no longer belong here, leave'. After turning around {character.character_name} sees nothing...", inline=False)
                        if character.stat_world_keys == 0:
                            embed.add_field(name="-", value=f"{character.character_name} understands that there's somewhere that they need to go, they can feel pressure holding them back\n+1 Englightenment", inline=False)
                            world_lock(1, character.stat_world_keys)

            if exploration_number == 2:
                energy_stones_gained = 1
                embed.add_field(name="-", value=f"{character.character_name} found an energy crystal in cave while passing by\n+{energy_stones_gained} Energy Stones", inline=False)

            if exploration_number == 3:
                embed.add_field(name="-", value=f"{character.character_name} was ambushed by a group of goblins", inline=False)
                ...
                if lose_chance > character.character_level:
                    ...
                    if run_chance <= character.character_level:
                        embed.add_field(name="-", value=f"{character.character_name} got away", inline=False)
                    else:
                        hp_lost = hp_suppresion(30, member)
                        xp_gained = xp_suppresion(10, member)
                        embed.add_field(name="-", value=f"{character.character_name} was defeated, but {character.character_name} took one down with them\n-{hp_lost}%hp +{xp_gained}xp", inline=False)
                else:
                    hp_lost = hp_suppresion(5, member)
                    xp_gained = xp_suppresion(30, member)
                    embed.add_field(name="-", value=f"{character.character_name} sent those goblins to the underworld, {character.character_name} was scratched\n+{xp_gained}xp -{hp_lost}%hp", inline=False)

            if exploration_number == 4:
                energy_stones_gained = 5
                embed.add_field(name="-", value=f"{character.character_name} saw a passing merchant and after selling off a strange stick {character.character_name} found, the merchant gave {character.character_name} some energy stones.\n+{energy_stones_gained} Energy Stones", inline=False)
            
            if exploration_number == 5:
                energy_stones_gained = random.randint(1,3)
                embed.add_field(name="-", value=f"Walking along the dirt road, a migrant tripped and dropped {energy_stones_gained} energy stone(s)... {character.character_name}'s lucky day\n+{energy_stones_gained} Energy Stones", inline=False)

            if exploration_number == 6:
                embed.add_field(name="-", value=f"A group of thugs passing by starts harassing {character.character_name}", inline=False)
                lose_chance = random.randint(1,10)
                if lose_chance > character.character_level:
                    hp_lost = hp_suppresion(40, member)
                    xp_gained = xp_suppresion(5), member
                    embed.add_field(name="-", value=f"They couldn't find anything valuable on {character.character_name}, so they beat {character.character_name} up\n-{hp_lost}%hp +{xp_gained}xp", inline=False)
                else:
                    energy_stones_gained = 3
                    hp_lost = hp_suppresion(10, member)
                    xp_gained = xp_suppresion(50, member)
                    embed.add_field(name="-", value=f"{character.character_name} beat them within an inch of their lives\n+{energy_stones_gained} Energy Stones -{hp_lost}%hp +{xp_gained}xp", inline=False)

            if exploration_number == 7:
                embed.add_field(name="-", value=f"{character.character_name} sees an old lady being yelled at by a middle aged man and she starts crying", inline=False)
                ladyEncounter = random.randint(1,2)
                if ladyEncounter == 1:
                    hp_lost = hp_suppresion(10, member)
                    xp_gained = xp_suppresion(15, member)
                    embed.add_field(name="-", value=f"{character.character_name} decided to get into a fight with the middle aged man and beats him up, only to hear about his speech impediment and that he was trying to cheer her up\n-{hp_lost}%hp +{xp_gained}xp", inline=False)
                elif ladyEncounter == 2:
                    embed.add_field(name="-", value=f"{character.character_name} ignores the yelling thinking that it's just part of human nature", inline=False)
            ...
            if exploration_number == 9:
                xp_gained = xp_suppresion(10, member)
                embed.add_field(name="-", value=f"A famous warrior walked into an inn {character.character_name} was staying at and tripped, they walk out after telling you never to wear high heels, to wear something comfortable\n+{xp_gained}xp", inline=False)

            if exploration_number == 10:
                xp_gained = xp_suppresion(15, member)
                embed.add_field(name="-", value=f"{character.character_name} found a caravan to join; after the trip {character.character_name} learned a lot\n+{xp_gained}xp", inline=False)

            if exploration_number == 11:
                forest_weeks_spent = random.randint(1,3)
                xp_gained = xp_suppresion(forest_weeks_spent * 7, member)
                embed.add_field(name="-", value=f"{character.character_name} spent {forest_weeks_spent} weeks in the woods\n+{xp_gained}xp", inline=False)

            if exploration_number == 12:
                embed.add_field(name="-", value=f"{character.character_name} participated in a nearby tournament", inline=False)
                ...
                if character.character_level >= final_odds:
                    xp_gained = xp_suppresion(10, member)
                    embed.add_field(name="-", value=f"{character.character_name} was the **Champion**\n+{xp_gained}xp -{hp_lost}%hp", inline=False)
                elif character.character_level >= semi_final_odds:
                    xp_gained = xp_suppresion(80, member)
                    embed.add_field(name="-", value=f"{character.character_name} won the semi finals\n+{xp_gained}xp -{hp_lost}%hp", inline=False)
                elif character.character_level >= quarter_final_odds:
                    xp_gained = xp_suppresion(60, member)
                    embed.add_field(name="-", value=f"{character.character_name} won the quarter finals\n+{xp_gained}xp -{hp_lost}%hp", inline=False)
                elif character.character_level >= second_round_odds:
                    xp_gained = xp_suppresion(40, member)
                    embed.add_field(name="-", value=f"{character.character_name} won the second round\n+{xp_gained}xp -{hp_lost}%hp", inline=False)
                elif character.character_level >= first_round_odds:
                    xp_gained = xp_suppresion(20, member)
                    embed.add_field(name="-", value=f"{character.character_name} won the first round\n+{xp_gained}xp -{hp_lost}%hp", inline=False)
                else:
                    xp_gained = xp_suppresion(10, member)
                    embed.add_field(name="-", value=f"{character.character_name} lost in the first round\n+{xp_gained}xp -{hp_lost}%hp", inline=False)

            # second world key ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            if exploration_number == 13:
                xp_gained = xp_suppresion(25, member)
                if character.character_level > 9:
                    embed.add_field(name="-", value=f"A wrinkled old man passed by {character.character_name} while preeching about the frustrations of mortals, {character.character_name} feels enlightened\n+{xp_gained}xp", inline=False)
                    if character.stat_world_keys == 1:
                            embed.add_field(name="-", value=f"{character.character_name} knows that they've broken the shackles of mortality and they need to go to new heights.\n+1 Englightenment", inline=False)
                            world_lock(2, character.stat_world_keys)
                else:
                    embed.add_field(name="-", value=f"A wrinkled old man passed by {character.character_name} while muttering something incomprehensible, {character.character_name} feels enlightened\n+{xp_gained}xp", inline=False)

            if exploration_number == 14:
                embed.add_field(name="-", value=f"{character.character_name} found a cave with a light shining through some vines", inline=False)
                cave_occupant_odds = random.randint(1,5)

                if cave_occupant_odds == 1:
                    embed.add_field(name="-", value=f"The occupant doesn't appreciate {character.character_name}'s snooping and attacks you", inline=False)
                    ...
                    if character.character_level >= withstand_chance:
                        ...
                        embed.add_field(name="-", value=f"{character.character_name} realizes too late that there is excess energy in their body and blacks out only to wake up in the middle of the forest\n-{hp_lost}%hp", inline=False)
            ...
            if exploration_number == 15:
                if not character.character_level > 9:
                    hp_lost = hp_suppresion(10, member)
                embed.add_field(name="-", value=f"A snow dragon warrior flew over {character.character_name} during their travels, it's a little cold\n-{hp_lost}%hp", inline=False)
                if character.stat_world_keys == 2:
                            embed.add_field(name="-", value=f"{character.character_name} follows the dragon warrior, finds that they've travelled through a portal. I wonder where this leads.\n+1 Englightenment", inline=False)
            ...
            if exploration_number == 16:
                ...
                if gender == 1:
                    ...
                    gender = "girl"
                xp_gained = xp_suppresion(10, member)
                embed.add_field(name="-", value=f"{character.character_name} watches as a young {gender} gets killed in a fight for resources, poor thing\n+{xp_gained}xp", inline=False)

            if exploration_number == 17:
                energy_stones_gained = random.randint(3,10)
                embed.add_field(name="-", value=f"{character.character_name} found a collector who collects well shaped Energy Stones, {character.character_name} pulls out their very best stone and sells it to him\n+{energy_stones_gained} Energy Stones", inline=False)

            if exploration_number == 18:
                hp_lost = hp_suppresion(5, member)
                embed.add_field(name="-", value=f"{character.character_name} was resting on a tree in a forest, a fairy flew by and scared {character.character_name} off the tree, ouch\n-{hp_lost}%hp", inline=False)

            if exploration_number == 19:
                xp_gained = xp_suppresion(15, member)
                embed.add_field(name="-", value=f"{character.character_name} passes by a nearby valley and learns about the history from a guide, there used to be 2 unique races here but they mutually destroyed themselves…\n+{xp_gained}xp", inline=False)

            if exploration_number == 20:
                xp_gained = xp_suppresion(5), member
                embed.add_field(name="-", value=f"{character.character_name} passed by a seemingly ordinary tree that begins talking about it's day with {character.character_name}... weird\n+{xp_gained}xp", inline=False)

            if exploration_number == 21:
                embed.add_field(name="-", value=f"During {character.character_name}'s travels they encounter a group of undercover demons", inline=False)
                capture_odds = random.randint(3,7)
                if capture_odds >= character.character_level:
                    hp_lost = hp_suppresion(15, member)
                    embed.add_field(name="-", value=f"{character.character_name} was captured, after being set free {character.character_name}'s head feels fuzzy\n-{hp_lost}hp", inline=False)
                else:
                    fight_odds = random.randint(7,10)
                    if fight_odds <= character.character_level:
                        xp_gained = xp_suppresion(50, member)
                        hp_lost = hp_suppresion(random.randint(5,15), member)
                        embed.add_field(name="-", value=f"{character.character_name} beat up the demons and found that they plan on taking over the nearby area, interesting\n-{hp_lost}hp +{xp_gained}xp", inline=False)
                    else:
                        embed.add_field(name="-", value=f"{character.character_name} fled and after being chased for awhile, {character.character_name} came upon a town and realized the demons disappeared.", inline=False)

            if exploration_number == 22:
                embed.add_field(name="-", value=f"{character.character_name} found a castle in the middle of nowhere, {character.character_name} travelled here after learning about it's invinciblity in a nearby inn.", inline=False)
                if not character.character_level >= 30:
                    hp_lost = hp_suppresion(5, member)
                    embed.add_field(name="-", value=f"{character.character_name} tries to punch the wall, ouch\n-{hp_lost}%hp", inline=False)
                else:
            ...
            if exploration_number == 23:
                ...
                embed.add_field(name="-", value=f"A person appears infront of {character.character_name}, slaps {character.character_name} and disappears... they wonder why that happened until they find that they feel sick.\n-{hp_lost}%hp +{energy_stones_gained} Energy Stones", inline=False)

            if exploration_number == 24:
                xp_gained = xp_suppresion(15, member)
                embed.add_field(name="-", value=f"{character.character_name} finds a group of people, dancing on a hill. They join in on the fun, how exhilerating!\n+{xp_gained}xp", inline=False)

            if exploration_number == 25:
                embed.add_field(name="-", value=f"During {character.character_name}'s travels they encounter a person that starts saying that they're possessed", inline=False)
        ...
        if dead:
            embed.add_field(name=f"Character Death", value=f"{character.character_name} died, but they don't know that they've died.", inline=False)
    ...
    async def stats(self, interaction: discord.Interaction):
        ...
        stat_energy_stones_gained = number_format(character.stat_energy_stones_gained)
    ...
    async def rankings(self, interaction: discord.Interaction):
        ...
        with sqlite3.connect('DB Storage/essence.db') as db:
            ...
            lb = cursor.fetchall()
            cursor.execute(f"SELECT character_name, character_level, character_xp FROM incremental WHERE member_id = ?", (member.id,))
