...
class Staff(commands.Cog, app_commands.Group):
    ...
    async def send_message(self, interaction: discord.Interaction, embed):
        ...
        await self.command_logs.send(embed=embed)
    ...
    async def task_checker(self):
        ...
        while not self.bot.is_closed():
            ...
            with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
                cursor = db.cursor()
                cursor.execute(
                    f"SELECT member_id, daily, daily_reminder, notifications FROM users ORDER BY daily DESC")
                ...
                for channel in guild.voice_channels:
                    for member in channel.members:
                        if member.voice.self_deaf or member.voice.afk:
                            ...
                            cursor.execute(
                                f"SELECT activity_points FROM users WHERE member_id = ?", (member.id,))
                            ...
                            cursor.execute(
                                f"SELECT apmulti FROM users WHERE member_id = ?", (member.id,))
                            activity_points_multiplier = cursor.fetchone()
                            activity_points_multiplier = activity_points_multiplier[0]
                            ...
                            val = (activity_points +
                                   (1 * activity_points_multiplier), member.id)
    ...
    async def timestamp(self, interaction: discord.Interaction, seconds_added: int = 0):
        ...
        embed = discord.Embed(
            description=f"Timestamp: {timestamp}, TT: <t:{timestamp}:t>", colour=0x800080)
    ...
    async def information(self, interaction: discord.Interaction):
        command_logs = self.bot.get_channel(976519708096467025)
        embed = discord.Embed(
            title="**Information**", description="Essence is a fantasy gaming discord, which promotes creating a group of friends to explore the online world", colour=0x800080)
        embed.add_field(
            name="**Town**", value="In the town category you can find many different lovers of fantasy games and talk or mess around.", inline=False)
        embed.add_field(
            name="**Parties**", value="Parties are a fundamental part of Essence and allow you to group up and compete against others in competitive and casual ways.")
        ...
        embed.add_field(name="**Game Categories**",
                        value="Essence has started guilds and teams in many different games, if you're interested head on over to <#737812517094555698>.", inline=False)
        embed.add_field(name="**Incremental RPG**",
                        value="Essence's developer is also working on creating games, you can check out the most recent one by doing $help in the direct messages of <@920471516817260564>", inline=False)
        ...
        embed.add_field(name="**Timestamps**",
                        value="Most timestamps will be converted to your local time with discord timestamps but otherwise the server is based in E.S.T. (UTC-4)", inline=False)
        ...
        embed.add_field(name="**Invite Link**",
                        value="Want to support Essence or invite your friends? Share our invite link! https://discord.gg/Fgyd22sB3g")
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await command_logs.send(embed=embed)
    ...
    async def rules(self, interaction: discord.Interaction):
        command_logs = self.bot.get_channel(976519708096467025)
        embed = discord.Embed(
            title="Rules of Essence", description="Essence Fantasy Gaming Community Discord Rules", colour=0x800080)
        embed.add_field(name="1. Basic Respect",
                        value="Basic respect should always be given, regardless of the topic- be that opinions, beliefs or problems.", inline=False)
        embed.add_field(
            name="2. Privacy", value="Privacy should also be respected at a basic level, what's said in dms/private should stay in dms/private.", inline=False)
        embed.add_field(
            name="3. Topics", value="The channel topics are there for a reason, try to stick to them as much as possible.", inline=False)
        embed.add_field(name="4. Brain Damage",
                        value="You have to support Josh with his brain damage, it's just a necessity. :wink:", inline=False)
        embed.add_field(name="5. Voice Chat",
                        value="Earrape, voice changers or soundboards are only allowed under the condition that it's not bothering the conversation.", inline=False)
        embed.add_field(
            name="6. PG-13", value="Although this hasn't been enforced much in the past, I would like to try to keep cursing and suggestive talk to a minimum.", inline=False)
        embed.add_field(
            name="7. Bots", value="Do not use the bots to irritate people on purpose, until Essence can completely substitute all bots- I have to ask this of you.", inline=False)
        embed.add_field(name="8. Attachments, embeds and gifs",
                        value="Big and spammy images can effect conversation, unless the attachment is relevant to the topic- they should be kept at a minimum.", inline=False)
        embed.add_field(name="9. Links and self-advertisement",
                        value="You're not allowed to advertise using this server as a medium unless you have explicit permission from staff or the user you're advertising to.", inline=False)
        embed.add_field(
            name="10. Safety", value="The internet is not a safe place, if you see something that doesn't look safe- please contact staff.")
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await command_logs.send(embed=embed)
    ...
    async def punish(self, interaction: discord.Interaction, member: discord.Member, reason: str):
        ...
        with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
            cursor = db.cursor()
            cursor.execute(
                f"SELECT user_punishments FROM users WHERE member_id = ?", (member_id,))
            ...
            db.commit()
            embed = discord.Embed(
                description=f"{member} has now been punished {user_punishments} times.", colour=0x800080, timestamp=datetime.utcnow())
            if user_punishments == 1:
                embed = discord.Embed(
                    description=f"Please don't break rules in Essence, {member.name}.\nVerbal Warning Reason: {reason}", colour=0xFF0000)
            elif user_punishments == 2:
                embed = discord.Embed(
                    description=f"Please stop breaking the rules in Essence, {member.name}.\nVerbal Warning Reason: {reason}", colour=0xFF0000)
            elif user_punishments == 3:
                embed = discord.Embed(
                    description=f"You are on timeout for 1 hour in Essence, {member.name}, please stop breaking the rules. \n1 Hour Timeout Reason: {reason}", colour=0xFF0000)
                await member.timeout(3600, reason=reason)
            elif user_punishments == 4:
                embed = discord.Embed(
                    description=f"You are on timeout for 8 hours in Essence, {member.name}. \n8 Hour Timeout Reason: {reason}", colour=0xFF0000)
                await member.timeout(28800, reason=reason)
            elif user_punishments == 5:
                embed = discord.Embed(
                    description=f"You are on timeout for 1 day in Essence, {member.name}. \n1 Day Timeout Reason: {reason}", colour=0xFF0000)
                await member.timeout(86400, reason=reason)
            elif user_punishments == 6:
                embed = discord.Embed(
                    description=f"You are on timeout for 1 week in Essence, {member.name}. \n1 Week Timeout Reason: {reason}", colour=0xFF0000)
                await member.timeout(604800, reason=reason)
            elif user_punishments == 7:
                embed = discord.Embed(
                    description=f"You have been permanently banned from Essence, {member.name}. \nBan Reason: {reason}", colour=0xFF0000)
    ...
    async def promote(self, interaction: discord.Interaction, arg: discord.Member = None, *, kwarg: str = None):
        ...
        db.commit()
        embed = discord.Embed(
            description=f"{member}'s party rank has been set to {kwarg}")
        await command_logs.send(f"{member}'s party rank has been set to {kwarg}", colour=0x800080)
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def purge(self, interaction: discord.Interaction, messages: int):
        ...
        embed = discord.Embed(
            description=f"{messages} messages have been deleted in {interaction.channel}!", colour=0x800080)
        await command_logs.send(embed=embed)
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def statistics(self, interaction: discord.Interaction):
        ...
        member = interaction.user
        embed = discord.Embed(
            description=f"**Server Statistics**", colour=0x800080)
        embed.add_field(
            name="Owner:", value=f"{interaction.guild.owner}", inline=False)
        embed.add_field(
            name="Members:", value=f"{interaction.guild.member_count}", inline=False)
        embed.add_field(
            name="Roles:", value=f"{len(interaction.guild.roles)}", inline=False)
        embed.add_field(
            name="Channels:", value=f"{len(interaction.guild.channels)}", inline=False)
        embed.add_field(name="Voice Channels:",
                        value=f"{len(interaction.guild.voice_channels)}", inline=False)
        embed.add_field(
            name="Categories:", value=f"{len(interaction.guild.categories)}", inline=False)
        embed.add_field(
            name="Creation:", value=f"{discord.utils.format_dt(interaction.guild.created_at)}", inline=False)
        embed.set_author(name=(f"{member.nick}'s Command"),
                         icon_url=member.display_avatar)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await command_logs.send(embed=embed)
    ...
    async def award(self, interaction: discord.Interaction, arg: discord.Member, kwarg: int):
        ...
        with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
            ...
            db.commit()
            if kwarg >= 1:
                await interaction.response.send_message(f"{member} has been awarded with {kwarg} party points.", ephemeral=True)
                await command_logs.send(f"{member} has been awarded with {kwarg} party points.")
            else:
                await interaction.response.send_message(f"{member} has lost {(kwarg * -1)} party points.", ephemeral=True)
                await command_logs.send(f"{member} has lost {(kwarg * -1)} party points.")
    ...
    async def event(self, interaction: discord.Interaction, arg: str = None):
        ...
        admin = discord.utils.get(
            interaction.guild.roles, id=795441220885151784)
        ...
        with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
            ...
            cursor.execute(
                f"SELECT party FROM users WHERE member_id = ?", (member_id,))
            party = cursor.fetchone()
            party = party[0]
            cursor.execute(
                f"SELECT event_participants FROM server WHERE party = ?", ("event",))
            event_participants = cursor.fetchone()
            event_participants = event_participants[0]
            cursor.execute(
                f"SELECT event_admins FROM server WHERE party = ?", ("event",))
        ...
        if arg == "roll":
            if admin in member.roles or mod in member.roles:
                ...
                if event == 1:
                    ...
                    embed = discord.Embed(
                        description=f"__**Rock paper scissors**__\n**Website**: https://www.rpsgame.org/\nSimple and fun, supervisor will set the rules and then you guys can go at eachother's throats! (ep will be distributed by the supervisor)", colour=0x800080)
            else:
                embed = discord.Embed(
                    description=f"You don't have permission to use this command.", colour=0x800080)
            embed.set_author(
                name=(f"{member.nick}'s Command"), icon_url=member.display_avatar)
            await interaction.response.send_message(embed=embed)
            await command_logs.send(embed=embed)

        else:
            embed = discord.Embed(
                description=f"There seems to be a problem with the kind of event you want to have.", colour=0x800080)
            embed.set_author(
                name=(f"{member.nick}'s Error"), icon_url=member.display_avatar)
            await interaction.response.send_message(embed=embed, ephemeral=True)
            await command_logs.send(embed=embed)
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        member_id = member.id
        visitor = discord.utils.get(
            member.guild.roles, id=(867097611323310082))
        timestamp = round(datetime.now().timestamp())
        ...
        if not party == None:
            ...
            if party == "Spirit":
                spirit = discord.utils.get(
                    member.guild.roles, id=(926660576980111370))
                await member.add_roles(spirit)
            if party == "Mana":
                mana = discord.utils.get(
                    member.guild.roles, id=(926660721054462083))
                await member.add_roles(mana)
            if party == "Aura":
                aura = discord.utils.get(
                    member.guild.roles, id=(926660747713474600))
                await member.add_roles(aura)
            if party == "Nature":
                nature = discord.utils.get(
                    member.guild.roles, id=(928466489588191322))
    ...
    async def on_message_delete(self, message: discord.Message):
        ...
        embed = discord.Embed(
            description=f"**Deleted Message from {message.author} in {message.channel}**:\n{message.content}", colour=0x800080)
    ...
    async def on_message(self, message: discord.Message):
        ...
        log_channel, log_channel2 = self.bot.get_channel(
            932351834734096494), self.bot.get_channel(969009985512157194)
        member = message.author
        timestamp = round(datetime.now().timestamp())
        config_line = 'spirit'
        guild_roles = [
            discord.utils.get(message.guild.roles,
                              id=867097611323310082),  # visitor 0
            discord.utils.get(message.guild.roles,
                              id=730626889709781043),  # guest 1
            discord.utils.get(message.guild.roles,
                              id=795491852533891112),  # member 2
            discord.utils.get(message.guild.roles,
                              id=865644597140258827),  # Honorary member 3
            discord.utils.get(message.guild.roles,
                              id=865644927114412073),  # vip 4
            discord.utils.get(message.guild.roles,
                              id=741152373921022092),  # mod 5
            discord.utils.get(message.guild.roles,
                              id=795441220885151784),  # admin 6
            discord.utils.get(message.guild.roles,
                              id=734838525144596493)  # robot 7
        ]
        ...
        with sqlite3.connect('Python/essence-bot/sensitive-data/essence.db') as db:
            cursor = db.cursor()
            cursor.execute(
                f"SELECT count, counter FROM server WHERE party = ?", (config_line,))
            counting_information = cursor.fetchone()
            count = counting_information[0]
            counter = counting_information[1]
            cursor.execute(
                f"SELECT story, writer FROM server WHERE party = ?", (config_line,))
            story_information = cursor.fetchone()
            story = str(story_information[0])
            writer = story_information[1]
            cursor.execute(
                f"SELECT user_count FROM users WHERE member_id = ?", (member.id,))
            ...
            activity_points = activity_points[0]
            cursor.execute(
                f"SELECT party_points FROM users WHERE member_id = ?", (member.id,))
            party_points = cursor.fetchone()
            party_points = party_points[0]
        ...
        if activity_points >= 100000 and party_points >= 100:  # vip
            if vip in member.roles or mod in member.roles or admin in member.roles:
                ...
                embed = default_embed(
                    description=f"<@{member.id}> has ranked up to VIP")
            ...
            if honorary_member in member.roles or vip in member.roles or mod in member.roles or admin in member.roles:
                ...
                embed = default_embed(
                    description=f"<@{member.id}> has ranked up to Honorary Member")
                await shout_out.send(embed=embed)
            ...
            if member_role in member.roles or honorary_member in member.roles or vip in member.roles or mod in member.roles or admin in member.roles:
                ...
                embed = default_embed(
                    description=f"<@{member.id}> has ranked up to Member")
            ...
            if guest in member.roles or member_role in member.roles or honorary_member in member.roles or vip in member.roles or mod in member.roles or admin in member.roles:
                ...
                embed = default_embed(
                    description=f"<@{member.id}> has ranked up to Guest")
            ...
            if visitor in member.roles or guest in member.roles or member_role in member.roles or honorary_member in member.roles or vip in member.roles or mod in member.roles or admin in member.roles:
                ...
                await member.add_roles(visitor)
                embed = default_embed(
                    description=f"<@{member.id}> has ranked up to Visitor")
                await shout_out.send(embed=embed, delete_after=5)

        # two word story ---------------------------------------------------------------------------------------------------------------------
        if message.channel == self.bot.get_channel(975885688681680986):
            if writer == member.id:
                await message.channel.set_permissions(message.guild.default_role, send_messages=False)
                embed = discord.Embed(
                    description=f"**{message.author} tried to hog the story**", colour=0x800080)
                ...
                if len(story.split()) > 249:
                    embed = discord.Embed(
                        description=f"**The story in two word story made it to**:\n{len(story.split())} words\n\n```{story}```", colour=0x800080)
                    await shout_out.send(embed=embed)
                await message.channel.set_permissions(message.guild.default_role, send_messages=True)
                ...
                await message.channel.set_permissions(message.guild.default_role, send_messages=False)
                embed = discord.Embed(
                    description=f"**{message.author} didn't say 2 words**", colour=0x800080)
                await message.channel.send(embed=embed, delete_after=3)
                await message.delete()
                await message.channel.set_permissions(message.guild.default_role, send_messages=True)
                ...
                await message.channel.purge(limit=2)
                embed = discord.Embed(
                    description=f"Current Story:```\n{story}```\nLast author: <@{member.id}>", colour=0x800080)
                await message.channel.send(embed=embed)
                await message.channel.set_permissions(message.guild.default_role, send_messages=True)

        # counting ---------------------------------------------------------------------------------------------------------------------
        if message.channel == self.bot.get_channel(975549476880154644):
            if user_count == None:
                user_count = 0
            if not message.content.isdigit():
                ...
                embed = discord.Embed(
                    description=f"**Non integer sent by {message.author}**:\n{message.content}", colour=0x800080)
                await message.channel.send(embed=embed)
                if count > 999:
                    embed = discord.Embed(
                        description=f"**The count in counting made it to**:\n{count}", colour=0x800080)
                    await shout_out.send(embed=embed)
                await message.channel.set_permissions(message.guild.default_role, send_messages=True)
                ...
                await message.channel.purge(limit=count + 10)
                embed = discord.Embed(
                    description=f"**Double input from {message.author}**:\n{message.content}", colour=0x800080)
                await message.channel.send(embed=embed)
                if count > 999:
                    embed = discord.Embed(
                        description=f"**The count in counting made it to**:\n{count}", colour=0x800080)
                ...
                await message.channel.purge(limit=count + 10)
                embed = discord.Embed(
                    description=f"**Wrong Number from {message.author}**:\n{message.content}", colour=0x800080)
                await message.channel.send(embed=embed)
                if count > 999:
                    embed = discord.Embed(
                        description=f"**The count in counting made it to**:\n{count}", colour=0x800080)
        ...
        # commands ----------------------------------------------------------------------------------------------------------------------------------------------------------------
        if message.channel == self.bot.get_channel(976345658644631623):
        ...
        if activity_points == None:  # activity points ----------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            cursor.execute(
                f"SELECT message_delay FROM users WHERE member_id = ?", (member.id,))
            activity_points = cursor.fetchone()
            if timestamp <= (activity_points[0] + 5):
                ...
                if apmulti[0] == None:
                    ...
                    cursor.execute(
                        f"SELECT activity_points FROM users WHERE member_id = ?", (member.id,))
                    activity_points = cursor.fetchone()
                    activity_points = activity_points[0] + 2
                    sql = "UPDATE users SET activity_points = ?, message_delay = ? WHERE member_id = ?"
                    val = (activity_points, timestamp, member.id)
                else:
                    cursor.execute(
        ...
        if message.channel == log_channel or message.channel == log_channel2:
            ...
            return
        embed = discord.Embed(
            description=f"**Message from {message.author} in {message.channel}**:\n{message.content}", colour=0x800080)
        await log_channel.send(embed=embed)
